...
export default function AdminHeader({ activeTab, setActiveTab, user }: HeaderProps) {
  const navLinks = [
  ...
  return (
    <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-8 fixed top-0 left-0 right-0 z-40 w-full shadow-sm">
      ...
      <div className="flex items-center gap-3">
        <span className="text-[12px] font-bold text-gray-700">{user?.fullName || user?.displayName || "Admin User"}</span>
